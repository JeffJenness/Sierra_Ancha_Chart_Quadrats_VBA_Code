Attribute VB_Name = "Margaret_maps"
Option Explicit

Public Sub DeclareWorkspaces_Maps(strOrigShapefiles As String, Optional strModifiedRoot As String, _
  ...
  strExportBase = "D:\arcGIS_stuff\consultation\Margaret_Moore\Sierra_Ancha\" & strDate & "\Quadrat_Map_Image_Files"
  strSetFolder = "D:\arcGIS_stuff\consultation\Margaret_Moore\Sierra_Ancha\" & strDate
  strExtractShapefileFolder = "D:\arcGIS_stuff\consultation\Margaret_Moore\Sierra_Ancha\" & strDate & "\Shapefile_Extractions_" & strCurrentDate
  strFinalFolder = "D:\arcGIS_stuff\consultation\Margaret_Moore\Sierra_Ancha\" & strDate & "\Final_Datasets_" & strCurrentDate

End Sub

Public Sub ExportImages_SA()
  ...
  Call DeclareWorkspaces_Maps(strOrig, , , strExportBase, strModifiedRoot)
  ...
  Dim varTempSites() As Variant
  Dim varTempSitesSpecific() As Variant
  Set pQuadratNumColl = FillQuadratNameColl_Rev_SA_Maps(strFinalQuadrats, , , varTempSites, varTempSitesSpecific)
  ...
  FillYearsAndQuadrats_Maps pDensityFClass, pCoverFClass, varYears, varQuads
  ...
  pProg.position = 0
  Dim strRevisedQuadName As String
  ...
  For lngYearIndex2 = 0 To UBound(varYears)
    ...
    For lngIndex = 0 To lngQuadCount - 1
      ...
      If MyGeneralOperations.CheckCollectionForKey(pQuadratNumColl, strQuadrat) Then
        varItem = pQuadratNumColl.Item(strQuadrat)
        strSite = varItem(0)
        strSiteSpecific = varItem(1)
        strPlot = varItem(2)
        strFolder = varItem(4)
        strFileHeader = varItem(5)
        strUTME = Format(CDbl(varItem(6)), "0")
        strUTMN = Format(CDbl(varItem(7)), "0")
        strComment = Format(CDbl(varItem(8)), "0.00000") & "°E, " & Format(CDbl(varItem(9)), "0.00000") & "°N"
        strElev = Format(CDbl(varItem(10)), "0")
      ...
      End If
      strRevisedQuadName = Replace(strSite, " ", "_") & "_" & strQuad
      ...
      If booExportThisOne Then
        ...
        If aml_func_mod.ExistFileDir(strOrig & "\" & strRevisedQuadName & "\" & strRevisedQuadName & "_" & strYear & "_C.shp") Or _
            aml_func_mod.ExistFileDir(strOrig & "\" & strRevisedQuadName & "\" & strRevisedQuadName & "_" & strYear & "_D.shp") Then
          ...
          If Not aml_func_mod.FileExists(strExportFilename) Then
            ...
            Set pFLayer = Margaret_maps.MakeGridFLayer_Maps
...
Public Sub ClearAnyInitialData()
  ...
  For lngIndex = 0 To pMxDoc.FocusMap.LayerCount - 1
    ...
    If pLayer.Name <> "Box" And pLayer.Name <> "Sample Grid" And pLayer.Name <> "" Then
      pLayers.Add pLayer
    End If
  Next lngIndex
...
Public Sub FillYearsAndQuadrats_Maps(pDensityFClass As IFeatureClass, pCoverFClass As IFeatureClass, _
    varYearsToFill() As Variant, varQuadratsToFill() As Variant)
  ...
  pSBar.ShowProgressBar "Extracting unique Year and Quadrat values...", 0, lngCount, 1, True
  pProg.position = 0

  Dim pDoneYears As New Collection
  Dim pDoneQuadrats As New Collection
  Dim lngQuadrats() As Long
  Dim lngYears() As Long
  Dim lngQuadratArrayIndex As Long
  Dim lngYearArrayIndex As Long
  ...
  Dim pRow As IRow
  Dim pQueryFilt As IQueryFilter
  varDatasets = Array(pDensityFClass, pCoverFClass)

  lngQuadratArrayIndex = -1
  lngYearArrayIndex = -1

  For lngDatasetIndex = 0 To UBound(varDatasets)
    Set pFClass = varDatasets(lngDatasetIndex)
    Set pQueryFilt = New QueryFilter
    pQueryFilt.AddField "Quadrat"
    pQueryFilt.AddField "Year"
    lngQuadratIndex = pFClass.FindField("Quadrat")
    lngYearIndex = pFClass.FindField("Year")
    Set pCursor = pFClass.Search(pQueryFilt, True)
    ...
    Do Until pRow Is Nothing
      ...
      strQuadrat = pRow.Value(lngQuadratIndex)
      If Not MyGeneralOperations.CheckCollectionForKey(pDoneQuadrats, strQuadrat) Then
        lngQuadrat = CLng(Replace(strQuadrat, "Q", ""))
        lngQuadratArrayIndex = lngQuadratArrayIndex + 1
        ReDim Preserve lngQuadrats(lngQuadratArrayIndex)
        lngQuadrats(lngQuadratArrayIndex) = lngQuadrat
        pDoneQuadrats.Add True, strQuadrat
      End If

      strYear = pRow.Value(lngYearIndex)
      If Not MyGeneralOperations.CheckCollectionForKey(pDoneYears, strYear) Then
        lngYearArrayIndex = lngYearArrayIndex + 1
        ReDim Preserve lngYears(lngYearArrayIndex)
        lngYears(lngYearArrayIndex) = CLng(strYear)
        pDoneYears.Add True, strYear
      End If
  ...
  QuickSort.LongAscending lngYears, 0, lngYearArrayIndex
  QuickSort.LongAscending lngQuadrats, 0, lngQuadratArrayIndex

  Dim lngIndex As Long
  Debug.Print "Found the following years..."
  ReDim varYearsToFill(lngYearArrayIndex)
  For lngIndex = 0 To lngYearArrayIndex
    varYearsToFill(lngIndex) = Format(lngYears(lngIndex), "0")
    Debug.Print "..." & Format(lngIndex + 1, "0") & "] Adding Year '" & varYearsToFill(lngIndex) & "'..."
  Next lngIndex

  ReDim varQuadratsToFill(lngQuadratArrayIndex)
  Debug.Print "Found the following quadrats..."
  For lngIndex = 0 To lngQuadratArrayIndex
    varQuadratsToFill(lngIndex) = "Q" & Format(lngQuadrats(lngIndex), "0")
    Debug.Print "..." & Format(lngIndex + 1, "0") & "] Adding Quadrat '" & varQuadratsToFill(lngIndex) & "'..."
  Next lngIndex
...
Public Sub MakePageNumbers(Optional pPageNumberByPlotDate As Collection)
  ...
  Set pQuadratNumColl = FillQuadratNameColl_Rev_SA_Maps(strFinalQuadrats, pPlotToQuadratConversion, pQuadratToPlotConversion, _
  ...
  Dim strModifiedRoot As String
  Dim strOrig As String
  Call DeclareWorkspaces_Maps(strOrig, strModifiedRoot, , strExportBase)
  ...
  For lngIndex = 0 To pFiles.Count - 1
    ...
    varItems = pQuadratNumColl.Item(Format(lngQuad, "0"))
    strSiteSpecific = varItems(1)
  ...
  Dim str2018_2019Report As String
  Dim p2018_2019PageByNameColl As New Collection
  For lngIndex = 0 To UBound(strFileNames)
    strJustName = varSortBySite(3, lngIndex)
    p2018_2019PageByNameColl.Add lngIndex + 1, strJustName

    If InStr(1, strJustName, "_2018", vbTextCompare) > 0 Then
      str2018_2019Report = str2018_2019Report & strJustName & vbTab & "p. " & Format(lngIndex + 1, "#,##0") & vbCrLf
    ElseIf InStr(1, strJustName, "_2019", vbTextCompare) > 0 Then
      str2018_2019Report = str2018_2019Report & strJustName & vbTab & "p. " & Format(lngIndex + 1, "#,##0") & vbCrLf & vbCrLf
    End If
  Next lngIndex
...
Public Function FillQuadratNameColl_Rev_SA_Maps(strQuadrats() As String, Optional pPlotToQuadratConversion As Collection, _
    Optional pQuadratToPlotConversion As Collection, Optional varSites As Variant, _
    Optional varSitesSpecific As Variant, Optional booRestrictToSite As Boolean = False, _
    Optional strSiteToAnalyse As String = "") As Collection

  Dim strPath As String
  strPath = "D:\arcGIS_stuff\consultation\Margaret_Moore\Sierra_Ancha\Summary_Data_from_JSJ\" & _
      "For_FillQuadratNameColl_Rev_SA.xlsx"
  ...
  Dim lngQuadratIndex As Long
  Dim lngUTMEIndex As Long
  Dim lngUTMNIndex As Long
  Dim lngLongIndex As Long
  Dim lngLatIndex As Long
  Dim lngElevIndex As Long
  Dim lngCommentIndex As Long

  Set pTable = pWS.OpenTable("Sierra_Ancha$")
  lngSiteIndex = pTable.FindField("Site")
  lngSiteSpecificIndex = pTable.FindField("Site_Specific_Name")
  lngPlotIndex = pTable.FindField("Plot")
  lngQuadratIndex = pTable.FindField("Quadrat")
  lngUTMEIndex = pTable.FindField("NAD83_UTM_Zone12_E")
  lngUTMNIndex = pTable.FindField("NAD83_UTM_Zone12_N")
  lngLongIndex = pTable.FindField("NAD83_Longitude")
  lngLatIndex = pTable.FindField("NAD83_Latitude")
  lngElevIndex = pTable.FindField("Elevation")
  ...
  Set pRow = pCursor.NextRow
  Do Until pRow Is Nothing
    strSite = Trim(pRow.Value(lngSiteIndex))
    If strSite <> "" Then
      strSiteSpecific = Trim(pRow.Value(lngSiteSpecificIndex))
      If (Not booRestrictToSite) Or InStr(1, strSite, strSiteToAnalyse) Then
        strPlot = Trim(pRow.Value(lngPlotIndex))
        strQuadrat = Trim(pRow.Value(lngQuadratIndex))
        dblUTME = pRow.Value(lngUTMEIndex)
        dblUTMN = pRow.Value(lngUTMNIndex)
        dblLongitude = pRow.Value(lngLongIndex)
        dblLatitude = pRow.Value(lngLatIndex)
        dblElev = pRow.Value(lngElevIndex)
        strComment = pRow.Value(lngCommentIndex)
        ...
        If strSite = "Natural Drainages" Then
          strFileHeader = "ND"
        ElseIf strSite = "Natural Drainages Watershed A" Then
          strFileHeader = "ND_A"
        ElseIf strSite = "Natural Drainages Watershed B" Then
          strFileHeader = "ND_B"
        ElseIf strSite = "Natural Drainages Watershed C" Then
          strFileHeader = "ND_C"
        ElseIf strSite = "Natural Drainages Watershed D" Then
          strFileHeader = "ND_D"
        ElseIf strSite = "Plot 1" Then
          strFileHeader = "Plot_1"
        ElseIf strSite = "Plot 3" Then
          strFileHeader = "Plot_3"
        Else
        ...
        ReDim varItem(11)
        varItem(0) = strSite
        varItem(1) = strSiteSpecific
        varItem(2) = strPlot
        varItem(3) = strQuadrat
        varItem(4) = strFolder
        varItem(5) = strFileHeader
        varItem(6) = dblUTME
        varItem(7) = dblUTMN
        varItem(8) = dblLongitude
        varItem(9) = dblLatitude
        varItem(10) = dblElev
        varItem(11) = strComment
        pReturn.Add varItem, strQuadrat
        ...
        End If
      End If 'End checking if restrict to site
    End If
    Set pRow = pCursor.NextRow
  Loop
  ...
  Set FillQuadratNameColl_Rev_SA_Maps = pReturn
...
ClearMemory:
  ...
  Set pDoneSitesSpecifics = Nothing
  Set pRow = Nothing
  Set pCursor = Nothing
...
Sub CreateAndApplyGridRenderer_Maps(pLayer As IFeatureLayer, strFieldName As String)
...
Public Function MakeGridFLayer_Maps() As IFeatureLayer
  ...
  Set MakeGridFLayer_Maps = pFLayer
  CreateAndApplyGridRenderer_Maps pFLayer, "Class"
