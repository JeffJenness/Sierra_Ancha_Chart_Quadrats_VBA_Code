...
Public Sub RecreateSubsetsOfConvertedDatasets_SA()
  ...
  Do Until pCoverFeature Is Nothing
    UpdateCount lngCounter, pProg, lngCount, "Cover"
  ...
  lngQuadratIndex = pDensityFClass.FindField("Quadrat")
  lngYearIndex = pDensityFClass.FindField("Year")
  ...
  Do Until pDensityFeature Is Nothing
    ...
    If pDensityFeature.Value(lngQuadratIndex) = "Q1" Then
      If pDensityFeature.Value(lngYearIndex) = 2021 Then
        DoEvents
      End If
    End If

    UpdateCount lngCounter, pProg, lngCount, "Density"
...
Public Sub UpdateCount(lngCounter As Long, pProg As IStepProgressor, lngCount As Long, strType As String)

  lngCounter = lngCounter + 1
  pProg.Step
  If lngCounter Mod 1000 = 0 Then
    pProg.Message = "Pass 1:  Transferring " & strType & " features [" & Format(lngCounter, "#,##0") & " of " & _
        Format(lngCount, "#,##0") & "]..."
    DoEvents
  End If
...
Public Sub FillMetadataItems(strAbstract As String, strBaseString As String, strPurpose As String)
  ...
  strAbstract = "This dataset consists of 33 permanent 1-m2 quadrats located near Sierra Ancha, Arizona, USA.  Individual plants in " & _
      ...
      "growth forms; (5) an inventory of the years each quadrat was sampled; (6) Counts of each species recorded at each site and quadrat, " & _
      "(7) tree density and basal area records " & _
      "for overstory plots that surround each quadrat, (8) TIFF and PDF maps of all sites and years (n = 1,523 maps), " & _
      "and (9) all species extracted into single-species shapefiles (with separate shapefiles for Site, Quadrat and Year) " & _
...
Public Sub RepairOverlappingPolygons_SA()
...
Public Sub SummarizeSpeciesByCorrectQuadrat_SA()
  ...
  strLine = """Species_Code"",""Species_Name"",""Common_Name"",""Cover_Density"",""Growth_Form"",""Native"",""C3_C4"",""Annual_Perennial"","
  For lngIndex = 0 To UBound(strAllSites)
  ...
  For lngIndex = 0 To UBound(strAllSpecies)
    ...
    Else
      ...
      strLine = """" & CStr(varVegArray(9)) & """,""" & strSpecies & """,""" & CStr(varVegArray(4)) & """,""" & CStr(varVegArray(3)) & """," & _
          """" & CStr(varVegArray(8)) & """,""" & CStr(varVegArray(7)) & """,""" & CStr(varVegArray(6)) & """,""" & CStr(varVegArray(5)) & ""","
    End If
...
Public Sub SummarizeSpeciesBySite_SA()
...
Public Sub SummarizeYearByCorrectQuadratByYear_SA()
  ...
  For lngIndex = 0 To UBound(strAllSites)
    ...
    If MyGeneralOperations.CheckCollectionForKey(pCommentColl, strSite) Then
...
Public Function ReturnCollectionOfYearsSurveyedByQuadrat(lngYear1 As Long, lngYear2 As Long) As Collection
  ...
  For lngIndex = 0 To pFiles.Count - 1
    ...
    Call FillQuadratAndYearFromDatasetBrowsename(strPath, strQuadrat, strYear, strCorD)
