...
Public Function ExportToCSV_SpecialCases_SA(pFLayerOrStandaloneTable As IUnknown, strFilename As String, _
    booOverwrite As Boolean, booForceUniqueFilename As Boolean, booOnlySelected As Boolean, _
    booSucceeded As Boolean, Optional varFieldNames As Variant = Null, _
    Optional pApp As IApplication = Nothing, Optional booSkipOID As Boolean = True, _
    Optional booCreateAreaAndCentroidFields = False, _
    Optional pQuadratColl As Collection) As String ' pPlotLocColl As Collection) As String
  ...
  If Not (TypeOf pFLayerOrStandaloneTable Is IStandaloneTable Or _
          ...
          TypeOf pFLayerOrStandaloneTable Is ITable) Then
    booSucceeded = False
    ExportToCSV_SpecialCases_SA = "Invalid Data Type:  This function requires either a feature class or table"
    GoTo ClearMemory
  End If
  ...
  If TypeOf pFLayerOrStandaloneTable Is IFeatureLayer Then
    ...
    If booOnlySelected Then
      ...
      If lngCount = 0 Then
        booSucceeded = False
        ExportToCSV_SpecialCases_SA = "No features selected in feature class"
        GoTo ClearMemory
      End If
      pSelSet.Search Nothing, False, pFCursor
    Else
  ...
  ElseIf TypeOf pFLayerOrStandaloneTable Is IStandaloneTable Then
    ...
    If booOnlySelected Then
      ...
      If lngCount = 0 Then
        booSucceeded = False
        ExportToCSV_SpecialCases_SA = "No rows selected in table"
        GoTo ClearMemory
      End If
      pSelSet.Search Nothing, False, pCursor
    Else
  ...
  If aml_func_mod.ExistFileDir(strFilename) Then
    ...
    ElseIf booOverwrite Then
      Kill strFilename
    Else
      ExportToCSV_SpecialCases_SA = "File Exists"
      booSucceeded = False
      GoTo ClearMemory
    End If
  End If
  ...
  Else
    For lngIndex = 0 To UBound(varFieldNames)
      ...
      If lngFieldIndex = -1 Then lngFieldIndex = pFields.FindFieldByAliasName(strFieldName)
      If lngFieldIndex = -1 Then
        ExportToCSV_SpecialCases_SA = "Unable to find Attribute Field '" & strFieldName & "'"
        booSucceeded = False
        GoTo ClearMemory
      End If
      Set pField = pFields.Field(lngFieldIndex)
  ...
  If lngFieldCounter = -1 Then
    ExportToCSV_SpecialCases_SA = "No Exportable Fields Found"
    booSucceeded = False
    GoTo ClearMemory
  End If
    ...
    Do Until pFeature Is Nothing
      ...
      If booCreateAreaAndCentroidFields Then
        ...
        If strQuad = "6" Then
          DoEvents
        End If
        varArray = pQuadratColl.Item(strQuad)
        dblEasting = varArray(6)
        dblNorthing = varArray(7)
  ...
  booSucceeded = True
  ExportToCSV_SpecialCases_SA = "Succeeded"
...
Public Sub AddValueToStringArray(strValue As String, strArray() As String, booOnlyIfNewValue As Boolean)
  ...
  Dim lngIndex As Long
  If MyGeneralOperations.IsDimmed(strArray) Then
    If booOnlyIfNewValue Then
      booValExists = False
      For lngIndex = 0 To UBound(strArray)
        If StrComp(strValue, strArray(lngIndex), vbTextCompare) = 0 Then
          booValExists = True
          Exit For
        End If
      Next lngIndex
      If Not booValExists Then
        ReDim Preserve strArray(UBound(strArray) + 1)
        strArray(UBound(strArray)) = strValue
      End If
    Else
      ReDim Preserve strArray(UBound(strArray) + 1)
      strArray(UBound(strArray)) = strValue
...
Public Sub AddValueToLongArray(lngValue As Long, lngArray() As Long, Optional booOnlyIfNewValue As Boolean = False)

  Dim booValExists As Boolean
  Dim lngIndex As Long
  If MyGeneralOperations.IsDimmed(lngArray) Then
    If booOnlyIfNewValue Then
      booValExists = False
      For lngIndex = 0 To UBound(lngArray)
        If lngValue = lngArray(lngIndex) Then
          booValExists = True
          Exit For
        End If
      Next lngIndex
      If Not booValExists Then
        ReDim Preserve lngArray(UBound(lngArray) + 1)
        lngArray(UBound(lngArray)) = lngValue
      End If
    Else
      ReDim Preserve lngArray(UBound(lngArray) + 1)
      lngArray(UBound(lngArray)) = lngValue
...
Public Sub AddValueToDoubleArray(dblValue As Double, dblArray() As Double, Optional booOnlyIfNewValue As Boolean = False)

  Dim booValExists As Boolean
  Dim lngIndex As Long
  If MyGeneralOperations.IsDimmed(dblArray) Then
    If booOnlyIfNewValue Then
      booValExists = False
      For lngIndex = 0 To UBound(dblArray)
        If dblValue = dblArray(lngIndex) Then
          booValExists = True
          Exit For
        End If
      Next lngIndex
      If Not booValExists Then
        ReDim Preserve dblArray(UBound(dblArray) + 1)
        dblArray(UBound(dblArray)) = dblValue
      End If
    Else
      ReDim Preserve dblArray(UBound(dblArray) + 1)
      dblArray(UBound(dblArray)) = dblValue
...
Public Sub AddValueToVariantArray(varValue As Variant, varArray() As Variant)

  If MyGeneralOperations.IsDimmed(varArray) Then
