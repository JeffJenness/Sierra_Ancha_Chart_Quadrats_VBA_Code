...
Public Sub ExportSubsetsOfSpeciesShapefiles_v2(Optional booDoAll As Boolean = False, Optional booDo10 As Boolean = False, _
  ...
  If booCreateOtherPolygon Then
    strExtractShapefileFolder = Replace(strExtractShapefileFolder, "Shapefile_Extractions", "Shapefile_Extractions_w_Other")
  End If
  ...
  Dim pAltFClass As IFeatureClass
  Dim pAltPolygon As IPolygon
  Dim pUnion As IPolygon
  Dim pArray As esriSystem.IVariantArray
  Dim varAltAttributes() As Variant
  Dim varAltFieldIndexArray() As Variant
  ...
  For lngIndex = 0 To UBound(varSpeciesArray)
    ...
    Do Until pFClass Is Nothing
      ...
      If pDataset.BrowseName <> "Cover_All" And pDataset.BrowseName <> "Density_All" Then
        ...
        booIsCover = InStr(1, pDataset.BrowseName, "cover.", vbTextCompare) > 0
        If booIsCover Then
          strAltName = Replace(pDataset.BrowseName, "Cover.", "Density.", , , vbTextCompare)
        Else
          strAltName = Replace(pDataset.BrowseName, "Density.", "Cover.", , , vbTextCompare)
        End If
        Set pAltFClass = pFeatWS.OpenFeatureClass(strAltName)
                    ...
                    For lngYearIndex = lngStartYear To lngEndYear
                      ...
                      Else
                        If booSurveyedThisYear Then
                          ...
                          varAltFieldIndexArray = varFieldIndexArray
                          Set pSrcFCursor = pFClass.Search(pQueryFilt, False)
                          Set pSrcFeature = pSrcFCursor.NextFeature
                          ...
                          If booCreateOtherPolygon Then
                            ...
                            Set pAltPolygon = ReturnOtherPolygon(pAltFClass, pReverseQueryFilt, varAltFieldIndexArray, varAltAttributes)

                            If Not pReversePolygon.IsEmpty And Not pAltPolygon.IsEmpty Then
                              Set pArray = New esriSystem.varArray
                              pArray.Add pReversePolygon
                              pArray.Add pAltPolygon
                              Set pReversePolygon = MyGeometricOperations.UnionGeometries3(pArray)
                            ...
                            ElseIf pReversePolygon.IsEmpty And Not pAltPolygon.IsEmpty Then
                              For lngIndex4 = 0 To UBound(varAltFieldIndexArray, 2)
                                pDestFBuffer.Value(varAltFieldIndexArray(3, lngIndex4)) = varAltAttributes(lngIndex4)
                              Next lngIndex4
                              Set pDestFBuffer.Shape = pAltPolygon
                              pDestFCursor.InsertFeature pDestFBuffer
                            ElseIf Not pReversePolygon.IsEmpty And pAltPolygon.IsEmpty Then
                              For lngIndex4 = 0 To UBound(varFieldIndexArray, 2)
                                pDestFBuffer.Value(varFieldIndexArray(3, lngIndex4)) = varAttributes(lngIndex4)
                              Next lngIndex4
                              Set pDestFBuffer.Shape = pReversePolygon
              ...
              If pFClass.FeatureCount(pQueryFilt) > 0 Then
                ...
                If lngPlotArrayIndex > -1 Then     ' export separate shapefiles by year, making folders for each plot
                  For lngIndex3 = 0 To lngPlotArrayIndex
                    ...
                    lngPlotCounter = lngPlotCounter + 1
                    Debug.Print "        Quadrat " & Format(lngPlotCounter, "0") & "] Working on Quadrat '" & strPlot & "'"
                    strReport = strReport & "        Plot " & Format(lngPlotCounter, "0") & "] Working on Plot '" & strPlot & "'" & vbCrLf
                    DoEvents
